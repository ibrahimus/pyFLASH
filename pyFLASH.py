...
BBMAP_OPTIONS = "usejni=f vslow=t maxindel=100k sam=1.3 threads=16"
BBMERGE_OPTIONS = "mininsert=30 usejni=f"
...
try:
	import sambedpure as sbp
except ImportError:
	print "Sambedpure not found. This may not be a problem."

current_dir = os.getcwd()+"/"

def processFastqFiles(forward, reverse, barcodes, file_type, bbmerge, ignore, chunk, mapping, gzipp, reps_merged, script_process, gzipafter, user_barcode, hamming_distance):
	...
	folders = ["barcode_sorted", "final_files", "final_files/bams", "final_files/bam_coverage", "final_files/bam_coverage/bedGraphs", "final_files/bam_coverage/bigwigs", "final_files/xlinks",
				...
				"barcode_sorted/bowtie2/cleaned", "barcode_sorted/bowtie2/unmapped", "barcode_sorted/bbmap", "barcode_sorted/bbmap/log", "barcode_sorted/bbmap/cleaned", 
	...
	if len(index_uniq) != len(index_barcodes):
		printStatus("Something is wrong with barcodes! Are they all uniqe?")
		sys.exit(1)


	# Fill these in as reqired. Should also change the available options down at the main() function, --mapping option.

	indices = {'genome':['path/to/bowtie2_index','bbmap_index_designator_as_build_number','path/to/chrom.sizes'],
				'hg19':['/data/akhtar/group/ibonomics/hg19', 1, '/data/akhtar/group/ibonomics/hg19.chrom.sizes'],
				'dm3':['/data/akhtar/group/ibonomics/dm3', 2, '/data/akhtar/group/ibonomics/dm3.chrom.sizes'], 
				'dm6':['/data/akhtar/group/ibonomics/dm6', 6, '/data/akhtar/group/ibonomics/dm6/dm6.chrom.sizes'],
	...
	if user_barcode.lower() == 'iclip':
		...
		left_user_barcode, right_user_barcode = None, 'NNYYNXXXXXXNN'
	else:
	...
	left_params = (None, None, None, None, None)
	right_params = (None, None, None, None, None)
	...
	print "Here's the barcode structure of forward reads: ", left_user_barcode
	print "Here's the barcode structure of reverse reads: ", right_user_barcode
	print "Here are the left parameters", left_params
	print "Here are the right parameters", right_params
	print "****************************************"
	...
	if not ignore: #ignore means "ignore unmerged reads."
		if script_process['BarcodeSplittingUnmerged'] == 'Finished':
			...
			printStatus("Barcode splitting has been initiated before but not completed. Deleting previos files and restarting.")
	...
	if bbmerge:
		if script_process['BarcodeSplittingMerged'] == 'Finished':
			printStatus("Merged reads were split successfully in a previos run. Skipping.")
			pass
	...
	if merged_statistics:
		...
		cmd_bad_barcodes = "sort bad_barcodes_merged.txt | uniq -c > bad_barcodes_merged_with_counts.txt"
		subprocess.check_call(cmd_bad_barcodes, shell=True)
		cmd_bad_barcodes_R1 = "sort bad_barcodes_R1.txt | uniq -c > bad_barcodes_R1_with_counts.txt"
	...
	if mapping:
		...
		if script_process['bowtie2'] == 'Finished':
			...
			runBowtie2(merged_files, script_process, indices, mapping)
		...
		if script_process['bbmap'] == 'Finished':
			...
			runBBMap(unmapped_reads, script_process, indices, mapping, merged_files)
		...
		if script_process['sambedcleaner'] == 'Finished':
			...
			runSamBedCleaner(bowtie_bams, bbmap_bams, script_process, chunk, index_barcode_names)
	...
	if reps_merged:
		if script_process['MergeReps'] == 'Finished':
			...
			RepMerger(index_barcode_names, script_process, indices, mapping)
...
def main():
	parser = argparse.ArgumentParser(description='Demultiplexer for FLASH data. Takes in fastq files, list of barcodes, throws out the reads separated for their barcode.')
	...
	parser.add_argument('-gz', '--gzip', action='store_true', help='Gzip the fastq files on the fly to save space. Slower.')
	...
	parser.add_argument('-ub', '--user_barcode', type=str, help='User defined internal barcode. For iCLIP it would be L=NNNXXXXNN; for uvCLAP it is L=NNNXXXXXNN,R=YRRYN; for FLASH it is R=NNYYNXXXXXXNN. Default is FLASH',
		default='FLASH')
	...
	"bowtie2":None, "bbmap":None, "sambedcleaner":None, "MergeBowtie2BBMap":None, "MergedSort":None, "MergeReps":None, "GenerateCov":None}
	...
	if args.bbmerge:
		if script_process['BBMerge'] == 'Finished':
			...
			processFastqFiles(name_forward, name_reverse, args.barcodes, filetype, args.bbmerge, args.ignore, args.chunk*100000, args.mapping, args.gzip, args.reps_merged, script_process, args.gzipafter, args.user_barcode, args.hamming_distance+1)
			...
			RunBBMerge(script_process, args, current_dir, name_forward, name_reverse)
			print "\n"
		...
		processFastqFiles(args.forward, args.reverse, args.barcodes, filetype, False, False, args.chunk*100000, False, args.gzip, False, script_process, args.gzipafter, args.user_barcode, args.hamming_distance+1)
...
def RunBBMerge(script_process, args, current_dir, name_forward, name_reverse):
	...
	BBMERGE_COMMAND = "%s in1=%s in2=%s out=%s outu1=%s outu2=%s" %(BBMERGE_OPTIONS, os.path.abspath(args.forward), os.path.abspath(args.reverse), 
		current_dir+"bbmap_merged.fastq.gz", current_dir+name_forward, current_dir+name_reverse)
	cmd = "cd %s && ./bbmerge.sh %s" %(BBMAP_PATH, BBMERGE_COMMAND)
	subprocess.check_call(cmd, shell=True)
...
def GenerateBinary(user_barcode):
	"""Takes in a string that is composed of only y or r letters
	such as 'yrry' (lower case), returns two sets that contain all possible combinations
	of such a string in DNA language, index of the binary barcode as a list, where the first elemenent
...
def UnmergedSplitter(forward, reverse, gzipp, script_process,index_barcode_names, index_barcodes, chunk, left_params, right_params, file_type, left_user_barcode, right_user_barcode, hamming_distance):
	...
	while True:
		try:
			...
			if left_params[4] and right_params[4]:
...
def runBowtie2(merged_files, script_process, indices, mapping):
	counting_files = 1
	...
	for i in merged_files:
		...
		cmd = "bowtie2 -p 16 -x %s --un-gz %s -U %s 2> %s | samtools view -b - > %s" %(indices[mapping][0], 
...
def runBBMap(unmapped_reads, script_process, indices, mapping, merged_files):
	counting_files2 = 1
	total_files = len(merged_files)
	for i in unmapped_reads:
		cmd = "cd %s && ./bbmap.sh %s build=%s in=%s out=%s 2> %s" %(BBMAP_PATH,
																	BBMAP_OPTIONS, 
																	indices[mapping][1],
																	current_dir+i, 
...
def runSamBedCleaner(bowtie_bams, bbmap_bams, script_process, chunk, index_barcode_names):
	jobs1 = []
	jobs2 = []
...
def RepMerger(index_barcode_names, script_process, indices, mapping):
